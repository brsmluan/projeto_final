...
void ssd1306_pixel(ssd1306_t *ssd, uint8_t x, uint8_t y, bool value) {
  ...
  uint16_t index = x + (y / 8) * ssd->width + 1;
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
  ...
  // Calcula o índice no array font
  if (c >= '0' && c <= '9') {
      index = (c - '0' + 1) * 8; // 0-9 começam em 8
  } else if (c >= 'A' && c <= 'Z') {
      index = (c - 'A' + 11) * 8; // A-Z começam em 88
  } else {
      return; // Caractere não suportado
  }

  // Desenha diretamente, sem rotação
  for (uint8_t i = 0; i < 8; i++) {
      uint8_t column = font[index + i];
      for (uint8_t j = 0; j < 8; j++) {
          bool pixel = (column >> j) & 0x01; // Lê bits de baixo para cima
          ssd1306_pixel(ssd, x + i, y + j, pixel); // i= coluna, j= linha
      }
  }
}
